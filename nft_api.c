...
json_t *nft_json_extract_array(struct nft_ctx *nft) {
	...
	if (nft_ctx_buffer_output(nft)
	    || nft_run_cmd_from_buffer(nft, list_cmd, sizeof(list_cmd)))
		return NULL;
...
int nft_json_is_exists(struct nft_ctx *nft, const char *family, const char *tb_name,
                       const char *ch_name) {
	...
	json_array_foreach(nft_array, index, value) {
		// Find table
		if (json_unpack_ex(value, &error, JSON_VALIDATE_ONLY, "{s{}}", "table")
		    == 0) {
		...
		// Find chain
		if (ch_name) {
			if (json_unpack_ex(value, &error, JSON_VALIDATE_ONLY, "{s{}}", "chain")
			    == 0) {
				...
				if (!strcmp(chname, ch_name) && !strcmp(tname, tb_name)
				    && !strcmp(chfamily, family))
					ex_chain = 1;
...
json_t *nft_json_add_table(const char *family, const char *tb_name,
                           json_error_t *err) {
	return json_pack_ex(err, 0, "{s{s{s:s, s:s}}}", "add", "table", "family", family,
	                    "name", tb_name);
}

json_t *nft_json_add_chain(const char *family, const char *tb_name,
                           const char *ch_name, const char *type, const char *hook,
                           const uint32_t prio, const char *policy,
                           json_error_t *err) {
	return json_pack_ex(err, 0, "{s{s{s:s, s:s, s:s, s:s, s:s, s:i, s:s}}}", "add",
	                    "chain", "family", family, "table", tb_name, "name", ch_name,
	                    "type", type, "hook", hook, "prio", prio, "policy", policy);
}

json_t *nft_json_build_expr_msq(const char *oifname) {
	json_t *nft_st_match = json_pack("{s{s{s:s}, s:s}}", "match", "left", "meta",
	                                 "oifname", "right", oifname);
...
json_t *nft_json_add_rule(const char *family, const char *tb_name,
                          const char *ch_name, const uint32_t handle, json_t *expr,
                          json_error_t *err) {
	json_t *jt_nft_rule =
	        json_pack_ex(err, 0, "{s{s{s:s, s:s, s:s, s:i, so}}}", "add", "rule",
	                     "family", family, "table", tb_name, "chain", ch_name,
...
int main() {
	...
	jt_nft_elem = nft_json_add_chain("ip", "mytable", "OUTPUT", "nat", "output", 200,
	                                 "accept", &err);
	if (json_array_append(jt_nft_array, jt_nft_elem) != 0) {
		fprintf(stderr, "JSON error: %s\n", err.text);
		rc = -1;
	...
	jt_nft_elem = nft_json_add_rule("ip", "mytable", "OUTPUT", 11, nft_expr, &err);
